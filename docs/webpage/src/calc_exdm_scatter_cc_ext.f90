...
contains
    ...
    subroutine exdm_scatter_cc_ext_calc(binned_rate, &
        ...
        eq_pos = matmul(PW_dataset%red_to_xyz, 1.0_dp*core_electron%eq_pos_red(core_electron%config(init_id, 1), :))
        ...
        if ( omega >= expt%E_threshold ) then
            ...
            do m = 1, dm_model%n_mX
                ...
                q_min = max(numerics%q_min, &
                            dm_model%mX(m)*dm_model%vX_max*(1.0_dp - sqrt(abs(1.0_dp - (omega/omega_kin_max)))))
                    ...
                    do q = 1, numerics%n_q
                        ...
                        do a = 1, size(numerics%q_angular_mesh, 1)
                            ...
                            do g = 1, PW_dataset%n_G
                                ...
                                form_fac = form_fac + conjg(wfc_FT_fkf(g))*&
                                                        wfc_i_FT*&
                ...
                ! interpolate/integrate to get b_rate
                do w = 1, bins%n_E
                    do t = 1, expt%n_time
                        do f = 1, dm_model%n_med_FF
    ...
    subroutine cc_ext_interpolate_dRdq_to_binned_rate(q_list, dRdq, b_rate, q_bin_width)
        ...
        ! go through all the bins (except last)
        if ( size(b_rate) > 1 ) then

            do q = 1, size(b_rate) - 1
                
                q_bin_L = (q - 1)*q_bin_width + eps
                q_bin_R = q*q_bin_width

                b_rate(q) = integrate_log_interpolate(q_list, dRdq, q_bin_L, q_bin_R)
