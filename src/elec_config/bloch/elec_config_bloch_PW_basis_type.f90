...
contains
    ...
    subroutine elec_config_bloch_PW_basis_type_initialize(self, exdm_inputs, type, &
        ...
        self%hdf5_file_id = hdf5_file_id

        ! make sure the dataset exists
        data_path = "elec_states/"//trim(adjustl(type))//"/bloch/PW_basis/"
        if ( hdf_exists(self%hdf5_file_id, trim(adjustl(data_path))) ) then

            ! state info
            call hdf_get_dims(self%hdf5_file_id, trim(adjustl(data_path))//"state_info/energy_list", dims)
            n_states = dims(1)
            ...
            call hdf_read_dataset(self%hdf5_file_id, trim(adjustl(data_path))//"state_info/energy_list", energy_list)

            allocate(i_list(n_states), source = 1)
            call hdf_read_dataset(self%hdf5_file_id, trim(adjustl(data_path))//"state_info/i_list", i_list)

            allocate(k_vec_red_list(n_states, 3), source = 0.0_dp)
            call hdf_read_dataset(self%hdf5_file_id, trim(adjustl(data_path))//"state_info/k_vec_red_list", k_vec_red_list)

            call hdf_read_dataset(self%hdf5_file_id, trim(adjustl(data_path))//"state_info/jac_list", self%jac_list)
            call hdf_read_dataset(self%hdf5_file_id, trim(adjustl(data_path))//"state_info/Zeff_list", self%Zeff_list)
            call hdf_read_dataset(self%hdf5_file_id, trim(adjustl(data_path))//"state_info/k_id_list", self%k_id_list)

            ! config
            call hdf_get_dims(self%hdf5_file_id, trim(adjustl(data_path))//"config/G_list_red", dims)
            allocate(self%G_list_red(dims(1), dims(2)), source = 0)
            allocate(self%G_list(dims(1), dims(2)), source = 0.0_dp)
            call hdf_read_dataset(self%hdf5_file_id, trim(adjustl(data_path))//"config/G_list_red", self%G_list_red)
            ...
            call hdf_get_dims(self%hdf5_file_id, trim(adjustl(data_path))//"state_info/u_FT_r/n_1", dims)
            spin_dof = dims(2)
            ...
            do n = 1, n_states
                ...
                ! PW basis
                self%states(n)%hdf5_file_id = self%hdf5_file_id
