module calc_exdm_scatter_vc
    ...
    interface dme_scatter_vc_calc
        module procedure dme_scatter_vc_calc_no_spin 
        module procedure dme_scatter_vc_calc_spin
    end interface

contains

    subroutine dme_scatter_vc_calc_spin(binned_rate_t,&
            wfc_FT_i, wfc_FT_f,&
            val_id, cond_id, &
            n_FFT_grid,&
            k_cut, &
        ...
        complex(dp) :: wfc_FT_i(n_k, n_in_G, 2)
        complex(dp) :: wfc_FT_f(n_k, n_in_G, 2)
        ...
        do ki = 1, k_cut
            ...
            do kf = 1, k_cut
                ...
                if ( ( E_bin_num >= E_bin_threshold ) .and. &
                    ...
                    matrix_ele = pauli_spin_matrix(0)

                    ! this looks slow... (reshape arrays to n x m so that single call to matmul can be used?)
                    do g3 = 1, n_FFT_grid(3)
                        do g2 = 1, n_FFT_grid(2)
                            do g1 = 1, n_FFT_grid(1)

                                T_if(g1, g2, g3) = dot_product(& 
                                        conjg(wfc_f(:, g1, g2, g3)),&
                                        matmul( matrix_ele, wfc_i(:, g1, g2, g3) )&
                                        )

                            end do
                        end do
                    end do
                    ! T_if = matmul( conjg(wfc_f), matmul( matrix_ele, wfc_i ) )
    ...
    subroutine dme_scatter_vc_calc_no_spin(binned_rate_t,&
            wfc_FT_i, wfc_FT_f,&
            val_id, cond_id, &
            n_FFT_grid,&
            k_cut, &
