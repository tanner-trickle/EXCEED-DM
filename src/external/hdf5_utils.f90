...
module hdf5_utils
  ...
  interface hdf_read_dataset
     ...
     module procedure hdf_read_dataset_string_0
...
contains
  ...
  subroutine hdf_read_vector_from_dataset_double(loc_id, dset_name, offset, vector)
    ...
    if (size(vector,1) == dset_dims(1)) then
       ...
       if (all(offset(1:rank-1) <= dset_dims(2:rank)) .and. all(offset(1:rank-1) > 0)) then
          ...
          call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, vector, mdims, hdferror, mspace_id, dspace_id)
  ...
  subroutine hdf_read_dataset_string_0(loc_id, dset_name, str)
    ...
    integer(HID_T), intent(in) :: loc_id              ! local id in file
    character(len=*), intent(in) :: dset_name         ! name of dataset
    character(len=*), intent(out) :: str               ! data to be written
    ...
    if (hdf_print_messages) then
       write(*,'(A)') "--->hdf_read_dataset_string_0: " // trim(dset_name)
    end if

    call h5ltread_dataset_string_f(loc_id, dset_name, str, hdferror)

    ! remove trailing null characters
    length = index(str, achar(0)) - 1
    str = str(:length)

  end subroutine

  subroutine hdf_write_dataset_string_0(loc_id, dset_name, str)
    ...
    integer(HID_T), intent(in) :: loc_id              ! local id in file
    character(len=*), intent(in) :: dset_name         ! name of dataset
    character(len=*), intent(in) :: str               ! data to be written

    integer :: hdferror

    if (hdf_print_messages) then
       write(*,'(A)') "--->hdf_write_dataset_string: " // trim(dset_name)
    end if

    call h5ltmake_dataset_string_f(loc_id, dset_name, str, hdferror)
