...
module m_config
  ...
  integer, parameter :: CFG_string_len = 1000 !< Fixed length of string type
...
999 continue ! If there was an error, the routine will end here
  ...
  subroutine CFG_write_markdown(cfg_in, filename, hide_unused, include_title)
    use iso_fortran_env
    type(CFG_t), intent(in)       :: cfg_in
    character(len=*), intent(in)  :: filename
    logical, intent(in), optional :: hide_unused
    ...
    logical, optional :: include_title
    ...
    do i = 1, cfg%num_vars
       ...
       case (CFG_real_type)
          do j = 1, cfg%vars(i)%var_size
             write(myUnit, ADVANCE="NO", ERR=998, FMT="(A,ES12.6)") &
                  " ", cfg%vars(i)%real_data(j)
             ! write(myUnit, ADVANCE="NO", ERR=998, FMT="(A)") &
             !      " ", cfg%vars(i)%real_data(j)
          end do
